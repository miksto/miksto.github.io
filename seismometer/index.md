...
_An additional note_
The above mentioned 2.5V bias voltage is based on the assumption assumes that the op-amp can drive the output voltage all the way down to 0V (ground) and up to 5V (power rail). In practice however, that is rarely the case, and the bias voltage should set to the middle point between the max and the min output voltages of the op-amp.
